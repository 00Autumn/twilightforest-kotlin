...
class TFEnglishLanguageProvider(output: FabricDataOutput, registryLookup: CompletableFuture<RegistryWrapper.WrapperLookup>) : FabricLanguageProvider(output, registryLookup) {
    ...
    override fun generateTranslations(p0: RegistryWrapper.WrapperLookup?, entries: TranslationBuilder?) {
        ...
        entries?.add(TFItems.COOKED_VENISON, "Cooked Venison")
        entries?.add(TFItems.RAW_MEEF, "Raw Meef")
        entries?.add(TFItems.COOKED_MEEF, "Cooked Meef")
        ...
        entries?.add(TFBlocks.ROOT_BLOCK, "Root Block")
        entries?.add(TFBlocks.LIVEROOT_BLOCK, "Liveroot Block")

        entries?.add(TFBlocks.IRONWOOD_BLOCK, "Block of Ironwood")
        entries?.add(TFBlocks.STEELEAF_BLOCK, "Block of Steeleaf")
        entries?.add(TFBlocks.KNIGHTMETAL_BLOCK, "Block of Knightmetal")
        entries?.add(TFBlocks.FIERY_BLOCK, "Block of Fiery Metal")
        entries?.add(TFBlocks.ARCTIC_FUR_BLOCK, "Block of Arctic Fur")

        entries?.add(TFBlocks.MAZESTONE, "Mazestone")
        entries?.add(TFBlocks.MAZESTONE_BRICK, "Mazestone Brick")
        entries?.add(TFBlocks.MAZESTONE_LARGE_BRICK, "Large Mazestone Brick")
        entries?.add(TFBlocks.CHISELED_MAZESTONE, "Chiseled Mazestone")
        entries?.add(TFBlocks.CUT_MAZESTONE, "Cut Mazestone")
        entries?.add(TFBlocks.DECORATIVE_MAZESTONE, "Decorative Mazestone")
        entries?.add(TFBlocks.CRACKED_MAZESTONE, "Cracked Mazestone")
        entries?.add(TFBlocks.MOSSY_MAZESTONE, "Mossy Mazestone")
        entries?.add(TFBlocks.MAZESTONE_MOSAIC, "Mazestone Mosaic")
        entries?.add(TFBlocks.MAZESTONE_BORDER, "Mazestone Border")

        entries?.add(TFBlocks.UNDERBRICK, "Underbrick")
        entries?.add(TFBlocks.MOSSY_UNDERBRICK, "Mossy Underbrick")
        entries?.add(TFBlocks.CRACKED_UNDERBRICK, "Cracked Underbrick")
        entries?.add(TFBlocks.UNDERBRICK_FLOOR, "Underbrick Floor")

        entries?.add(TFBlocks.TOWERWOOD, "Towerwood")
        entries?.add(TFBlocks.ENCASED_TOWERWOOD, "Encased Towerwood")
        entries?.add(TFBlocks.CRACKED_TOWERWOOD, "Cracked Towerwood")

        entries?.add(TFBlocks.HEDGE, "Hedge")
        entries?.add(TFBlocks.HEDGE_ROSE, "Flowering Hedge")

        entries?.add(TFBlocks.STRONGHOLD_SHIELD, "Stronghold Shield")

        entries?.add(TFBlocks.INFESTED_TOWERWOOD, "Infested Towerwood")
        entries?.add(TFBlocks.ANTIBUILT_BLOCK, "Antibuilt Block")
        entries?.add(TFBlocks.FAKE_GOLD, "Fake Gold")
        entries?.add(TFBlocks.FAKE_DIAMOND, "Fake Diamond")

        entries?.add(TFBlocks.MAZE_SLIME_BLOCK, "Maze Slime Block")

        entries?.add(TFBlocks.TWILIGHT_OAK_LOG, "Twilight Oak Log")
        entries?.add(TFBlocks.STRIPPED_TWILIGHT_OAK_LOG, "Twilight Oak Log")
        entries?.add(TFBlocks.TWILIGHT_OAK_LEAVES, "Twilight Oak Leaves")
        entries?.add(TFBlocks.TWILIGHT_OAK_SAPLING, "Twilight Oak Sapling")
        entries?.add(TFBlocks.TWILIGHT_OAK_PLANKS, "Twilight Oak Planks")
        entries?.add(TFBlocks.TWILIGHT_OAK_DOOR, "Twilight Oak Door")
        entries?.add(TFBlocks.TWILIGHT_OAK_TRAPDOOR, "Twilight Oak Trapdoor")
        entries?.add(TFBlocks.TWILIGHT_OAK_FENCE, "Twilight Oak Fence")
        entries?.add(TFBlocks.TWILIGHT_OAK_FENCE_GATE, "Twilight Oak Fence Gate")
        entries?.add(TFBlocks.TWILIGHT_OAK_STAIRS, "Twilight Oak Stairs")
        entries?.add(TFBlocks.TWILIGHT_OAK_SLAB, "Twilight Oak Slab")
        entries?.add(TFBlocks.TWILIGHT_OAK_PRESSURE_PLATE, "Twilight Oak Pressure Plate")
        entries?.add(TFBlocks.TWILIGHT_OAK_BUTTON, "Twilight Oak Button")
